...
def autolink(escaped: str) -> str:
    ...
    escaped = EMAIL.sub(lambda m: f'<a href="mailto:{m.group(0)}">{m.group(0)}</a>', escaped)
...
def link_count(path: Path) -> int:
    ...
    majors = len(re.findall(r"source: 'https?://", text))
    return len(re.findall(r"\]\(https?://", text)) + majors
...
def parsed_links(doc: dict) -> int:
    anchors = sum(len(re.findall(r'<a href="https?://', h)) for h in html_strings(doc))
    for section in doc["sections"]:
        for b in section["blocks"]:
            if b["t"] == "table":
                anchors += sum(1 for r in b["refs"] if r and "url" in r) + (len(b["rows"]) if b["sharedRef"] else 0)
            if b["t"] == "sources":
                anchors += len(b["links"])
            if b["t"] == "faculty":
                anchors += sum(1 for r in b["rows"] if r.get("source"))
    return anchors
...
def main() -> int:
    ...
    parser.add_argument("--slugs", default="aub,aust,usj")
    ...
    for slug in args.slugs.split(","):
        ...
        for locale in LOCALES:
            ...
            expected = link_count(path)
            got = parsed_links(doc)
            if got != expected:
                problems.append(f"{slug}/{locale}: {expected} links in MDX, {got} kept")
    ...
    print(f"wrote {args.out.relative_to(ROOT)} ({len(body) // 1024} KB)")
