...
#: Docusaurus plugin id per content kind — also the on-disk folder name.
PLUGIN_BY_KIND: dict[Kind, str] = {"university": "universities", "scholarship": "scholarships"}

#: The sidebar key each plugin's sidebars file exports.
...
def plan_versions(tree: ContentTree, only_year: str | None = None) -> VersionPlan:
    ...
    for kind in ("university", "scholarship"):
        ...
        # Oldest → newest so "most recent year at or before Y" is a running map.
...
def write_version_manifests(plan: VersionPlan, out_prefix: str = "") -> None:
    ...
    for plugin, years in plan.years_by_plugin.items():
        ...
        for year in years:
            ...
            # createVersionedSidebarFile), so the autogenerated stub is the
            # correct content — it resolves against the version's own dir.
            _write_json(
                out_prefix,
